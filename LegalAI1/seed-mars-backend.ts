...
import { processBatch } from '../lib/appeal-ai/batch-processor';
import { predictOutcome } from '../lib/appeal-ai/outcome-predictor';
import { extractArguments } from '../lib/appeal-ai/argument-extractor';
import { generateAllDrafts } from '../lib/appeal-ai/draft-generator';
...
async function seed() {
    console.log('Seeding 10 demo cases directly into the DB and AI Pipeline...');

    // Get first org ID to pretend we are logged in
    const firstOrg = await prisma.organization.findFirst();
    if (!firstOrg) {
        console.error("No organization found in DB. Please run the app and log in first.");
        process.exit(1);
    }
    const orgId = firstOrg.id;

    try {
        // 1. Process batch (Creates cases, classifies, routes, citations)
        console.log('Running processBatch() for 10 cases...');
        const processedCases = await processBatch(cases as any, orgId);
        console.log(\`✅ Batch created \${processedCases.length} cases.\`);

    // Complete the rest of the pipeline sequentially to avoid overwhelming moonshot
    for (const result of processedCases) {
      if (result.caseId) {
        console.log(\`\\n\\[Case \${result.caseNumber}\\] Continuing pipeline...\`);

        // Fetch case needed for drafts
        const appealCase = await prisma.appealCase.findUnique({ where: { id: result.caseId } });
        if (!appealCase) continue;

        // 2. Predict Outcome
        console.log('  - Predicting outcome...');
        const prediction = await predictOutcome(result.caseId, appealCase.denialText, appealCase.appealText!);
        await prisma.appealCase.update({
          where: { id: result.caseId },
          data: {
            predictionOutcome: prediction.predictedOutcome,
            predictionConfidence: prediction.confidence,
            predictionHighlights: prediction.keyFactors
          }
        });
        
        // 3. Extract Arguments
        console.log('  - Extracting arguments...');
        const fullText = appealCase.denialText + '\\n\\n' + appealCase.appealText;
        const extractedArgs = await extractArguments(fullText, result.caseId);
        
        if (extractedArgs.length > 0) {
          await prisma.appealArgument.createMany({
            data: extractedArgs.map(arg => ({
          ...
          const storedArgs = await prisma.appealArgument.findMany({ where: { caseId: result.caseId }, orderBy: { argumentIndex: 'asc' }});
          
          // 4. Generate Drafts via Agentic Retrieval
          console.log(\`  - Generating drafts safely... (\${storedArgs.length} args)\`);
          const drafts = await generateAllDrafts(storedArgs as any, appealCase.denialText, result.caseId);
          
          for (let i = 0; i < storedArgs.length; i++) {
             const draftRes = drafts[i];
             await prisma.appealDraft.create({
                 data: {
                     argumentId: storedArgs[i].id,
                     draftText: draftRes.draftText,
                     verdict: draftRes.verdict,
                     confidence: draftRes.confidence,
                     precedentEclis: draftRes.precedentEclis,
                     reasoning: draftRes.reasoning
                 }
             });
          }
          console.log('  ✅ Drafts and AI decisions saved.');
        }
      }
    }
    
    console.log('\\n🎉 All 10 demo cases fully processed and ready for the dashboard!');
